...
const NeweagePaidInternshipLetter = ({
  ...
  /* ================= SMARTMATRIX SALARY STRUCTURE ================= */

  const round2 = (num) => Math.round((Number(num) || 0) * 100) / 100;

  const monthlyStipend = round2(Number(data?.stipend ?? stipend) || 0);
  const annualStipend = round2(monthlyStipend * 12);

  /* SmartMatrix Percentage Structure */
  const basic = round2(monthlyStipend * 0.4);
  const hra = round2(monthlyStipend * 0.18);
  const da = round2(monthlyStipend * 0.12);
  const special = round2(monthlyStipend * 0.16);
  const food = round2(monthlyStipend * 0.06);

  /* Adjustment to avoid rounding mismatch */
  const misc = round2(monthlyStipend - (basic + hra + da + special + food));

  /* Salary Rows */
  const salaryRows = [
    { name: "Basic", monthly: basic },
    { name: "House Rent Allowance", monthly: hra },
    { name: "Dearness Allowance", monthly: da },
    { name: "Special Allowance", monthly: special },
    { name: "Food Allowance", monthly: food },
    { name: "Misc. Allowance", monthly: misc },
  ];

  /* Totals */
  const totalMonthly = monthlyStipend;
  const totalAnnual = annualStipend;
