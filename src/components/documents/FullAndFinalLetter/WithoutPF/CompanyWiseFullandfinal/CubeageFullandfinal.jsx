...
const CubeageFullAndFinal = ({ data = {}, company = {} }) => {
  ...
  const basicActual = Math.floor(monthlyCTC * 0.40);
  const hraActual = Math.floor(monthlyCTC * 0.18);
  const daActual = Math.floor(monthlyCTC * 0.16);
  const ltaActual = Math.floor(monthlyCTC * 0.12);
  const specialActual = Math.floor(monthlyCTC * 0.06);

  let allowActual = Math.floor(monthlyCTC * 0.08);
  const totalAfterRoundingActual = basicActual + hraActual + daActual + ltaActual + specialActual + allowActual;
  allowActual = allowActual + (monthlyCTC - totalAfterRoundingActual);
  const grandTotalActual = basicActual + hraActual + daActual + ltaActual + specialActual + allowActual;
  ...
  const basicEarned = Math.floor(earnedCTC * 0.40);
  const hraEarned = Math.floor(earnedCTC * 0.18);
  const ltaEarned = Math.floor(earnedCTC * 0.12);
  const specialEarned = Math.floor(earnedCTC * 0.06);
  const allowEarned = Math.floor(earnedCTC * 0.08);

  // Earned DA is exactly what is left after pulling out the fixed values from the total prorated earned salary
  const daEarned = Math.floor(earnedCTC) - (basicEarned + hraEarned + ltaEarned + specialEarned + allowEarned);
  const grandTotalEarned = basicEarned + hraEarned + daEarned + ltaEarned + specialEarned + allowEarned;
  ...
  /* ── Deductions (WithoutPF: no PF) ── */
  const pt = 200;
  const otherDed = 2000;
  const totalDeductions = (pt + otherDed);

  /* ── Net ── */
  const netPayable = Math.ceil(totalEarned - totalDeductions);
  ...
  return (
    ...
    <Box sx={{ width: "210mm", minHeight: "297mm", backgroundColor: "white", fontFamily: "'Calibri', 'Arial', sans-serif" }}>
      ...
      <Box sx={{ px: 4, py: 1 }}>
        ...
        <Table sx={{ width: "100%", borderCollapse: "collapse", marginTop: "30px", tableLayout: "fixed" }}>
          <TableBody>
            ...
            <TableRow>
              <TableCell colSpan={3} sx={C({ textAlign: "center" })}>L.T.A.</TableCell>
              <TableCell colSpan={1} sx={C({ textAlign: "center", color: "black" })}>{fmt(ltaActual)}</TableCell>
              <TableCell colSpan={1} sx={C({ textAlign: "center", color: "black" })}>{fmt(ltaEarned)}</TableCell>
            </TableRow>
            <TableRow>
            ...
            <TableRow>
              ...
              <TableCell colSpan={1} sx={C({ textAlign: "center", color: "black" })}>{fmt(specialActual)}</TableCell>
              <TableCell colSpan={1} sx={C({ textAlign: "center", color: "black" })}>{fmt(specialEarned)}</TableCell>
            </TableRow>
